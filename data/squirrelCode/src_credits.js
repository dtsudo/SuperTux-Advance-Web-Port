...
window.superTuxAdvanceWebVersion.squirrelFiles['src/credits.nut'] = function () { 
...
creditsLength = 0 ; 
creditsSprites =  [  ]  ; 
startCredits =  function ( folder = "res" ) { creditsData = jsonRead ( fileRead ( folder + "/credits.json" )  )  ; 
 if ( creditsData == null )  { 
...
y += creditsData [ "credits" ]  [ i ]  [ "h" ]  + 5 ; 
 break ;  case "spacing" : y += fontH * creditsData [ "credits" ]  [ i ]  [ "height" ]  ; 
 break ;  }  } 
...
creditsOffset -= getcon ( "up" , "hold" )  && creditsOffset >=  - 10 ? 2 : 0 ; 
 if ( creditsOffset >= screenH (  )  + creditsLength + 10 || getcon ( "pause" , "press" )  )  { 
  for (  var i = 0 ;
 i < creditsSprites . len (  )  ; i += 1 )  { 
  if ( creditsSprites [ i ]  == null ) continue ; 
