...
::textMenu <- function(){
	...
	if(menu.len() > menuMax) for(local i = cursorOffset; i < cursorOffset + menuMax; i++) {
		...
		if(cursorOffset > 0) for(local ii = 0; ii < 4; ii++) drawSprite(font2, 103, (screenW() / 2 - 24) + (ii * 12), screenH() - menuY - (fontH * (menuMax + 1)))
		// webBrowserVersionChange: avoid reusing the variable name i
		if(cursorOffset < menu.len() - menuMax) for(local ii = 0; ii < 4; ii++) drawSprite(font2, 98, (screenW() / 2 - 24) + (ii * 12), screenH() - menuY)
	}
